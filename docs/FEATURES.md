...
| Promotions Step (eligibility & Experience cap) | 🟡 | **Promotion was a switch, and nothing capped Experience.** `handleToggleElite` set `isElite` and asked nothing, so a Warband could promote an Amalgam — which the rulebook lists under *Models That Cannot Be Promoted* — and could promote its whole roster, when the book skips the step entirely at six ELITE models. Experience was granted with no ceiling, so a LIMITED POTENTIAL model passed the 7 it is allowed and went on earning Advancement Rolls it is not entitled to. Both bounds and both model tables are now derived from rulebook pp.105-111 into `campaign.promotions`, and every model in them resolves to a unit id at build time or the build fails — never matched by name, because the book's *Fly Thralls* is the catalogue's *Winged Thrall*. The switch is gated on `canBePromoted` and says which rule refuses it; the post-battle Experience award lists a capped model beside everyone else who earns nothing, with the reason. The cap is read from the model's LIMITED POTENTIAL **keyword**, not the rulebook's table, because `dispatch-01` replaces the Brazen Bull's Warband Entry with a keyword row that omits it — precedence working, and the build prints the difference. 🟡 until FD-06b brings the Promotion Dice Pool, its assignment rule and the five-miss counter, which is what decides whether an eligible model is actually promoted (RR-05) |
