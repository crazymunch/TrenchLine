...
| 5b — the `.ros` generator | **Landed, unverified in NewRecruit** | `services/rosterRos.ts` and `components/builder/NewRecruitExport.tsx`, documented in [`ROS-EXPORT.md`](ROS-EXPORT.md). E4 taken as written: fatal disables the download and offers the TrenchLine file, warnings and informational notes are shown. Round-tripped against the `.ros` NewRecruit wrote — every selection agrees on `entryId`, `entryGroupId`, `group` and `from`. Profiles, rules and categories are deliberately not written, which is the largest untested assumption and is warned about on every export. **E3's acceptance gate is uncleared**: nobody has opened one of these files |
