# Round 2 status — read §10 first

This file retains the original brief for traceability. [§10](#10-round-2--decision-record-and-replacement-design) supersedes disputed round-1 recommendations. No new sub-processor or expanded Cloudflare processing is permitted while Safe Browsing review is pending. Claude's source documents are unchanged.
...
## 10. Round 2 — decision record and replacement design

Date: 6 September 2026. Reviewed [Claude's response](LIVE-PLAY-CLAUDE-REVIEW.md) at `e4e8fdd1452e3f0e1caec5f626a87444be603b31`, branch `claude/project-handover-e0sj82` (PR #43), not main. §§1–9 above are retained as the historical first proposal; **this section supersedes conflicting recommendations**, especially §§3–4 command legality, §6 workload assumptions and §8 vendor-spike sequencing. “Accepted” below means accepted into this design recommendation, not owner authorization to implement.

### 10.1 Decision table

| Proposal | Evidence | Accepted / rejected / deferred | Owner decision needed |
| --- | --- | --- | --- |
| D1: opponent Blood spending is core interaction | Generated markers: Blood spentBy opponent, cap 6; Blessing controller, uncapped; rulebook pp.19–20 | Accepted; round-1 authority emphasis was wrong | No |
| D1: derive permission from marker data | Same source; contextual rules still matter | Accepted as baseline, not complete legality engine | Decide multiplayer beyond two seats later |
| D2: ledger, instant echo, human corrections | Physical table is external truth; software state may be incomplete | Accepted as recommended product behavior | Yes: approve ledger-first, not automated referee |
| D2: no battle-time validation | Membership, malformed targets, retries and revoked writers remain real | Rejected for protocol/security invariants; ordinary gameplay prerequisites become warnings | Confirm correction/override UX |
| D2/Q6: timestamp-window conflicts; “nothing replayed” | Clocks/latency do not establish causality; a log needs a projection | Rejected; explicit references and deterministic fold | No, implementation semantics |
| D3: extract match state/protocol before vendor | match.ts mutates combat fields on roster units; polling exists | Accepted | No vendor approval implied |
| D4: no new processor or widened Cloudflare role | Owner constraint and pending SB-1b | Accepted, hard gate | Clearance then explicit approval before changed processing |
| D5: neither action estimate load-bearing | 500 and 3,600 are assumptions, not measured events | Accepted; no cost-led provider choice | Later budget ceiling |
| Q2: existing Neon first | One durable store avoids room-to-campaign export boundary | Accepted for baseline prototype | Later transport choice |
| Q5: wounds/status/activation inherently safe offline | Absolute values, resets and stale writes are noncommutative | Rejected as blanket claim | Approve disconnected pending/disputed experience |
| Q7: one row/action and only sequence index | Durable deduplication needs unique action identity; sequencing needs concurrency control | Rejected as complete schema/cost model | No |
| Q9: 11–18 versus 9–16 weeks | Neither estimated from measured migration work | Deferred; extraction first, then re-estimate | Scope/budget, not calendar commitment |

### 10.2 D1 correction: source-derived entitlement and the real hot path

I was wrong to frame opponent-model spending as an exceptional permission. Read `markers[].spentBy` from the match's pinned dataset: Blood expenditure is assigned to the opponent, Blessing to the controller. Marker holdings belong to the target model. The Blood holdings cap is six; the Blessing record has no cap.

But Claude's “spend up to the published cap” is not quite the replacement either: a holdings cap is not current availability or a per-roll spending rule. `spentBy` and `cap` alone do not encode all timing, roll-context or modifier constraints. Keep contextual rule references, not a second handwritten restatement of the rulebook. Two-player “opponent” is unambiguous; teams/multiple opponents require a separately agreed relation.

The common action should capture marker type, model holding it, amount, relevant roll/resolution reference and authenticated actor. Do not require the model controller to tap confirmation for every entitled opponent spend. One announced physical outcome should have one recording actor, with correction available to the table; duplicate damage reporting is not solved by model ownership.

Measure tap-to-local-feedback, durable acknowledgement and remote visibility separately, focusing on alternating cross-player marker actions during a roll. Proposed local feedback target: within 100ms; remote p95 250–500ms is an aspirational trial target, not a measured guarantee. Do not claim the current two-second polling cadence meets it: polling delay alone can approach two seconds. The owner can accept a slower GM mirror without declaring it adequate Live Play.

### 10.3 D2 replacement: human-authoritative gameplay, server-authoritative record

I agree with ledger-first. I do not agree that attributed append-only storage alone gives convergence or that validation should wait until finalisation.

Separate three categories:

1. **Hard protocol boundaries:** identity, membership, role, match/target existence, schema/version, finite bounded quantities, action-ID integrity, authority epoch, lifecycle and privacy. Reject violations immediately. Never infer actor from submitted payload. These do not referee the physical game.
2. **Gameplay observations:** record what the players say occurred, even if the server's inferred rules state disagrees. Normal UI uses source-derived entitlements. A plainly attributed manual correction/override can record a house ruling or a missed marker without inventing official legality. Surface discrepancies rather than blocking the turn.
3. **Consequential commits:** round/reset coordination, final confirmation and campaign application have exact lifecycle/idempotency requirements. Strict transactions guarantee one recorded operation; they cannot prove honest physical dice or rules compliance.

A gameplay discrepancy and a rejected request need different UI. Do not silently clamp an overdrawn balance to zero and call the action successful. Keep the observation in the log, mark its projection disputed, show the last agreed value plus pending adjustment, and offer an explicit correction. Before implementation, specify this reducer behavior precisely.

Local echo is provisional and immediate. “Saved” means durably committed. “Seen by opponent” and “agreed by opponent” are distinct; presence does not prove either. A rejected unauthorized request is removed from provisional state with a clear explanation; an authenticated rules discrepancy stays as a recorded observation.

#### Minimal event envelope and commit path

Proposed fields: protocolVersion, matchId, actionId, clientInstanceId, clientSequence, authorityEpoch, observedSequence, eventType, typed payload, optional causation/reference IDs. Server supplies actor identity, canonical sequence and receivedAt. Client time is descriptive only.

In one database transaction:

- Check current membership/epoch/lifecycle and bounded event schema.
- Deduplicate by unique (matchId, actionId); bind receipt to actor and payload hash. Identical retry returns the original receipt; different content with the same ID is a protocol error.
- Allocate canonical match sequence safely (e.g. locked match row or equivalent transactional compare-and-swap); append event and update sequence/projection/checkpoint metadata as needed.
- Commit before acknowledgement. Delivery may fail without losing the event; clients fetch missing sequences.

Use unique (matchId, sequence) AND (matchId, actionId). Match membership also needs efficient authorization access. One event row per user-visible action is a good grouping choice, not proof of one billable write. Sequence allocation, snapshots, receipt storage and indexes still cost. Store dedup fields on the event when appropriate; no mandatory separate receipt table.

Polling consumes this same append API and cursor-based event read. The current whole-board mirror, which ignores baseRevision, cannot become multi-writer simply by admitting a second host. Never send whole-roster last-writer-wins replacements.

#### Corrections and causality

“Within a time window” produces false positives for legitimate consecutive spends and misses delayed/reconnected conflicts. Use target/resource identity, observed sequence, round/activation epoch and explicit action references. Some dependencies are unknowable because physical actions were never entered; do not imply complete automatic detection.

Example: A records spend E41; a lost response retries E41 (one event only). A second device submits E42 from the same old resource observation. Keep the distinct attributed observation but flag the balance conflict. Players append correction E43 referencing the mistaken entry and specifying the agreed resulting balance. E43 is not deletion of E42.

Define correction events as explicit adjustment or set-to-agreed-value with reason and references; never ambiguous toggle or “undo whatever happened last.” Later dependent entries may be marked disputed, not automatically reversed. Final confirmation covers a named sequence/hash and resolved disputes. Any subsequent accepted correction invalidates prior confirmation.

A deterministic fold is still required to reconstruct the board from ordered events and snapshots. That is replay in the technical sense; avoiding automatic gameplay re-adjudication is not avoiding replay. Pin reducer/protocol versions and migrate snapshots explicitly. Append-only means no in-place editing of game history, not exemption from privacy deletion or retention requirements.

### 10.4 Offline and handover are product limits, not CRDT magic

Notes can be kept privately offline. Recorded gameplay observations can be retained as a local pending journal, but must never be portrayed as already shared or universally safe to apply.

Claude's “additive wounds, status, activation” grouping is too broad: setting status, marking acted and resetting a round do not commute; wound limits/corrections make order significant; entitled marker spends can still be stale. On reconnect send original IDs and observed epochs, retain attribution, and reconcile discrepancies. Do not automatically apply old-round activation changes into a new round.

Round advance, takeover, finalisation and campaign export wait for online coordination. For long outages offer pause or explicitly continue on a designated single device; a real transfer of shared writing authority requires an acknowledged new epoch. An offline phone cannot revoke another offline phone. Preserve unsent observations for manual reconciliation, not silent loss.

Owner decision: whether the alpha permits disconnected observation entry at all. Recommended first cut: obvious offline indicator, local notes and preserved in-flight actions; pause new shared bookkeeping until reconnect. Add general offline journals only after reconnect/correction tests pass.

### 10.5 Aggregate extraction and scope

Accept Claude Q1's shared/local split. Match state includes participating roster snapshots and deployment, round/active side, model combat fields and eventually scenario/scores/weather. Panel visibility, searches, filters, card selection and modal state remain local.

Create match-scoped unit identities and immutable source roster snapshots. Do not keep authoritative mutable combat fields inside saved roster units. Build local-engine compatibility first, including migration/resume of an interrupted existing single-device battle. Test reset behavior and persistent injuries separately. Source-level coupling is confirmed; this review does not establish an observed production data-loss incident.

One engine / three adapters remains:
- Single Device: local durability and local writer.
- GM: one authenticated controlling seat; viewers receive permitted projection.
- Live Play: two editing seats with rule-derived ordinary actions and attributed corrections.

The first shared alpha: both forces, wounds/status/activation, Blood/Blessing bookkeeping, attributed log, turn counter, reconnect and corrections. No automated dice, scoring engine, private objectives or campaign rewards in this milestone. A turn counter still needs unambiguous advance/reset semantics. End-of-session archival can precede campaign reward integration.

### 10.6 Infrastructure and cost: revised recommendation

Start with existing Vercel/Neon HTTP endpoints and polling after aggregate extraction. Do not add messaging merely because it appears in the options table. Polling needs no publish outbox; if a broker is later added, use durable publication or fetch-on-notification with polling repair.

After a real two-player trace, compare:
- Existing polling: simplest state ownership; remote delay and read volume are its tradeoff.
- Existing Neon plus managed messaging: same durable store, additional delivery service and privacy gate.
- Cloudflare room: colocated coordination/socket service, but a second persistence/trust boundary for campaign export, deletion and operations.
- Dedicated sockets: only with explicit operational ownership; a persistent process does not eliminate durability.
- WebRTC: still not a shortcut around host loss, identity, signalling or shared record.

All new-provider production spikes and Cloudflare live-state use are blocked while SB-1b is pending. This is an owner-imposed sequencing constraint, not a claim about what Google requires or that approval is guaranteed. Clearance does not itself authorize purchasing/deploying; privacy, retention, region and budget decisions still follow.

Cloudflare may fit Free, but neither estimate proves it. Round 1's 3,600 actions versus Claude's 500 is a 7.2× difference; “about an order of magnitude” is directionally fair. Three/minute for 180 minutes gives 540. Neither is a sourced action rate, and Field Strength does not establish event counts, bursts, spectators or write amplification.

Record actions/match, burst rate, viewers, payload size, logical database operations and provider-billed meters separately. Two watchers polling every two seconds for three hours still make roughly 10,800 GETs independent of how often models act; 304 avoids body transfer, not necessarily request/database work. Stop/reduce polling when appropriate, but handle phone wake and visibility changes.

The earlier provider pricing section is historical context, not a purchase/capacity recommendation. Recheck published allowances at the transport decision. Do not recommend Paid club-night hosting merely from the discarded workload model; do not promise free production from the lower estimate either.

### 10.7 Revised delivery and acceptance gates

1. Inventory/extract state; migration fixtures and single-device regression tests.
2. Agree ledger/override, offline and reset semantics; versioned event schema and deterministic reducer.
3. Existing-stack GM then two-writer vertical slice, including retries/corrections and permissions.
4. Observe real table sessions; instrument marker-spend latency, action bursts, reconnect, read/write cost and user tolerance.
5. Only after clearance and explicit approval, compare transport options against measured needs.
6. Add result confirmation and campaign application as a separately gated transaction workflow.

Do not double-count extraction hidden inside both engine and alpha estimates. Both previous week ranges remain unvalidated planning guesses; re-estimate after package 1.

Replace round-1 gate 3 (“competing spends cannot both succeed”) with: distinct conflicting observations remain attributed, never silently duplicate or corrupt agreed balances, and can be resolved deterministically. Retain hard idempotency and security gates.

Required tests before shared release:
- Same action/retry/lost ACK applies once; same ID/different payload fails.
- Concurrent unique actions receive unique ordered sequence; gaps and reconnect converge.
- Rules discrepancy is visible and correctable without a network-dependent referee loop.
- Late old-round action, stale correction and old writer epoch cannot silently replace current agreed state.
- One marker action's local echo, durable save and remote visibility are separately observable.
- Revoked participant/spectator cannot write; server projections exclude private data on snapshots and history.
- Phone sleep/reload/deployment and schema-version mismatch recover honestly.
- Corrections with known dependents become disputed; finalisation cannot use stale confirmations.
- Repeated result application awards once; failure leaves a visible pending result; deletion prevents resurrection.
- Existing offline/local games and roster exports do not acquire transient shared state.
- Quota/outage does not fabricate synchronisation; retention/deletion tested.

### 10.8 Next decisions for owner and Claude

Owner: approve ledger-first with explicit overrides, choose offline-alpha policy, confirm two authenticated editing seats initially, and decide whether the observed polling delay is acceptable. Vendor and spending decisions remain deferred.

Claude: challenge the concrete projection of an overdrawn observation; propose correction and round-reset payloads with three adversarial traces; specify the source-derived entitlement plus manual override boundary; identify the exact migration needed for active saved games. Those are more valuable next than another vendor comparison.
