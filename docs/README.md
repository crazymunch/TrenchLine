...
| [`CHRONICLE.md`](CHRONICLE.md) | The Chronicle of Battles: what a finished match leaves behind, why a record holds names rather than references, and why no result is stored. |
...
| [`TRENCH-COMPANION.md`](TRENCH-COMPANION.md) | Trench Companion, the community app that is now the officially supported builder: how it works (a React bundle with the ruleset embedded, a WordPress backend, a public share endpoint per warband), what it does better, what TrenchLine does better, three directions and the recommendation. |
| [`DESIGNS-2026-09-25.md`](DESIGNS-2026-09-25.md) | The designs that follow the Trench Companion review, packed as fewer, larger pull requests: the parked queue with a no-built-in-admin fix (pack D); import from a Companion share link, a public share page and per-warband ruleset conversion (pack E); and the shape of what waits on the campaign-side demo. Starts with the push discipline. |
