...
| `TRENCHLINE_ADMIN_EMAILS` | no, but read the row | **Admin is a deployment setting and nothing else.** Unset, nobody is an administrator by address and the admin screens are closed to everyone; the server starts and serves every other request normally, and `src/instrumentation.ts` prints the reason once at startup. This row said exactly that before ADM-1 and it was not true: `env.ts` carried a built-in default address, so an unset variable granted admin to whoever that address belonged to — in every fork and every clone. There is no default now. Persisted grants still work — see [`DATABASE.md`](DATABASE.md#the-administrator-role); being able to drop this variable entirely is the point of AUTH-1. |
