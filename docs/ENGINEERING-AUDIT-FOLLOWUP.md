...
| API-02 — unvalidated and unbounded bodies | **Closed** | `src/lib/api/parse.ts` measures bodies, applies strict Zod schemas, bounds values and JSON, and returns controlled error messages. *This row read "Closed" while **four routes never went through it**: `auth/register`, `auth/reset`, `auth/verify` and `POST /api/warbands` all called `req.json()`, which parses before anyone can object to the size. Field-level limits are not a substitute — they run after the parse, and the parse is the work being bounded. The roster route was the sharpest case, since `units` and `armoryStash` are Json columns with no ceiling of their own. All four go through `readJson` now, with the cap asserted on each.* |
