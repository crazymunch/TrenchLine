...
~1.2 MB per ruleset, against 2.7 MB for the dataset itself. Only an exporter or
an importer reads it, and the app is used at a table on a phone — folding it
into `trenchline.generated.ts` would put every byte of it in the bundle for a
feature almost nobody uses there. `loadRosterPaths()` imports it dynamically,
the same way `app/api/dataset/route.ts` loads the dataset.

Paths are indices into `segments` because the ids are 19 characters each and
repeat constantly; interning them is what keeps the file at 1.2 MB rather than
4. The build writes it indented but with each path on one line, so a changed
path reads as a changed line in review.
